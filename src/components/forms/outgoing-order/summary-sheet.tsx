...
export function SummarySheet({
  ...
  const groupedBags = useMemo(() => {
    ...
    Object.entries(orderGroups).forEach(([orderId, bags]) => {
      ...
      const varietyGroups = Object.groupBy(bags, (b) => b.variety);

      Object.entries(varietyGroups).forEach(([variety, vBags]) => {
        if (!vBags?.length) return;
        ...
        <div className="flex-1 overflow-y-auto space-y-8">
          ...
          {selectedBags.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {Object.entries(Object.groupBy(groupedBags, (g) => g.orderId)).map(
                ([orderId, groups]) => {
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {first.order.createdAt
                              ? format(new Date(first.order.createdAt), 'MMM dd, yyyy')
                              : 'N/A'}
                          </p>
                        </div>
          ...
          <div className="rounded-lg border bg-muted/40 p-4 space-y-2">
            <div className="flex justify-between">
              ...
              <p className="text-lg font-bold text-primary">
                {selectedBags.reduce((s, b) => s + b.quantity, 0).toFixed(1)}
              </p>
            </div>

            <p className="text-xs text-muted-foreground border-t pt-2">
              {selectedBags.length} unique bag(s) across {groupedBags.length} gate passes
            </p>
          </div>
